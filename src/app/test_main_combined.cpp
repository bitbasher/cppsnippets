...
int main(int argc, char *argv[]) {
    ...
    QWidget window;
    window.setWindowTitle("TEST: Combined Tabs");
    ...
    qDebug() << "Step 4: Creating dummy container for both tabs...";
    QTabWidget* testTabs = new QTabWidget(&window);
    qDebug() << "✓ Test container created\n";
    
    qDebug() << "Step 5a: Calling setupTemplatesTab()...\n";
    try {
        mainWindow->setupTemplatesTab(testTabs);
        qDebug() << "\n✓✓✓ setupTemplatesTab() SUCCEEDED! ✓✓✓\n";
    } catch (const std::exception& e) {
        qCritical() << "\n✗✗✗ EXCEPTION in setupTemplatesTab():" << e.what() << "\n";
        return 1;
    } catch (...) {
        qCritical() << "\n✗✗✗ UNKNOWN EXCEPTION in setupTemplatesTab()\n";
        return 1;
    }
    
    qDebug() << "Step 5b: Calling setupExamplesTab()...\n";
    try {
